#!/usr/bin/env python3
"""Reproduce exact legacy HORIPAD FourSelect and add raw-page readiness tracing."""
...
def repl(s, old, new, label):
    n = s.count(old)
    if n != 1:
        raise SystemExit(f"r388 {label}: anchor count {n}, expected 1")
    return s.replace(old, new, 1)


s = CPP.read_text(encoding="utf-8")
# Exact hardware-positive FourSelect transform from 0bd8a4d workflow.
s = repl(s,
'''static void switchBuildHoripad(uint8_t slot, uint8_t out[8])
{
	uint32_t b = g_in[slot].buttons;
''',
'''static void switchBuildHoripad(uint8_t slot, uint32_t suppressButtons, uint8_t out[8])
{
...
{
	(void)k;
	// Exact legacy FourSelect topology: enumerate all four existing HORIPAD HIDs.
...
{
	// Exact hardware-positive FourSelect behavior: all four HIDs are mounted and
	// neutral; one selector routes the live Steam Controller to one HID.
	int bond = -1;
	for (uint8_t u = 0; u < g_usbMountCount; u++) {
		if (g_usbToBond[u] >= 0) {
			bond = g_usbToBond[u];
	...
	for (uint8_t u = 0; u < maxSlots(); u++) {
		...
		usbTxHid(&g_switch[u], 0, p, 8); // unchanged proven send path
	}
	h4TraceService();
}
'''
s = repl(s, old_task, new_task, "task")

# Trace implementation is inserted before task(), leaving proven HID behavior alone.
trace = r'''
// r388 trace-only positive control. Raw page is outside app + InternalFS; magic
// is committed last so interrupted writes never validate as a snapshot.
static constexpr uint32_t H4_TRACE_PAGE = 0x000EB000UL;
static constexpr uint32_t H4_TRACE_MAGIC = 0x38543448UL; // "H4T8"
...
static_assert(sizeof(H4TraceRecord) == 8, "r388 record size");
...
static void h4TraceAppend(uint8_t hid, uint8_t phase, bool ready, int selected)
{
	...
	r.ms = millis(); r.hid = hid; r.phase = phase; r.ready = ready ? 1 : 0;
...
static void h4Erase()
{
	NRF_NVMC->CONFIG = NVMC_CONFIG_WEN_Een; while (!NRF_NVMC->READY) {}
	NRF_NVMC->ERASEPAGE = H4_TRACE_PAGE; while (!NRF_NVMC->READY) {}
	NRF_NVMC->CONFIG = NVMC_CONFIG_WEN_Ren; while (!NRF_NVMC->READY) {}
}
static void h4Write(uint32_t addr, uint32_t value)
{
	NRF_NVMC->CONFIG = NVMC_CONFIG_WEN_Wen; while (!NRF_NVMC->READY) {}
	*(volatile uint32_t *)addr = value; while (!NRF_NVMC->READY) {}
	NRF_NVMC->CONFIG = NVMC_CONFIG_WEN_Ren; while (!NRF_NVMC->READY) {}
}
static bool h4Valid(uint32_t *count)
{
	...
	if (h->magic != H4_TRACE_MAGIC || h->version != H4_TRACE_VERSION || h->count > 32)
		return false;
	const H4TraceRecord *r = (const H4TraceRecord *)(H4_TRACE_PAGE + sizeof(H4TraceHeader));
...
static void h4Store()
{
	uint32_t count = g_h4TraceCount, sum = h4Checksum(g_h4Trace, count);
	h4Erase();
...
{
	if (!g_h4StartMs) g_h4StartMs = millis();
	if (!g_h4Stored && g_h4TraceCount && (uint32_t)(millis() - g_h4StartMs) >= 15000u)
		h4Store();
}
void switchHoriTraceClear()
{
	h4Erase(); g_h4TraceCount = 0; g_h4Seen = 0; g_h4StartMs = millis(); g_h4Stored = false;
}
void switchHoriTraceDump()
{
	uint32_t count = 0;
	if (!h4Valid(&count)) { Serial.println("# JT no valid HORIPAD raw trace"); return; }
	const H4TraceRecord *r = (const H4TraceRecord *)(H4_TRACE_PAGE + sizeof(H4TraceHeader));
	Serial.printf("# JT begin bytes=%lu records=%lu source=HORIPAD-FourSelect-r388-raw\n",
		      (unsigned long)(count * sizeof(H4TraceRecord)), (unsigned long)count);
...
h += "\n// r388 diagnostic raw-trace accessors.\nvoid switchHoriTraceDump();\nvoid switchHoriTraceClear();\n"
...
'''			} else if (!strcmp(line, "JT")) {
				...
				Serial.println("# JT HORIPAD raw trace cleared");
			} else if (!strcmp(line, "CD")) {
''', "serial commands")
SER.write_text(ser, encoding="utf-8")
print("r388 exact legacy FourSelect + trace-only raw observer applied")
